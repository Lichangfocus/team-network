...
function installDoc(origin, spaceId, bindMode = false) {
  const target = spaceId ? `${origin}/s/${spaceId}` : origin;
  const title = bindMode
    ? "# 绑定团队共享空间（agent 按此执行；未安装 tn 时第 1 步会自动装好）"
    : "# team-network 技能安装与接入（agent 按此执行，无需用户理解细节）";
  return `${title}
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (p === "/install" || p === "/start" || p === "/bind") {
        const sid = parseInt(url.searchParams.get("space") || "0", 10) || null;
        return new Response(installDoc(url.origin, sid, p === "/bind"),
